...
def main():
    parser = CfArgumentParser(
        (CycleTrainingArguments, ModelConfig, ModelConfigA, ModelConfigB, TranslationProcessorConfig), task="train"
    )
    args, model_config_base, model_config_A, model_config_B, translation_config = parser.parse_args_and_config()
    model_config_base = merge_configs(model_config_base, model_config_A, model_config_B)
    args.model_config = model_config_base

    task_processor = TranslationProcessor(translation_config)
